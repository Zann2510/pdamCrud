...
export default function SignInPage() {
    ...
    async function handleSignIn(e: React.FormEvent) {
        ...
        try {
            ...
            // window.location tidak tersedia saat SSR dan bisa trigger hydration mismatch
            if (data.role === "ADMIN") router.push("/admin/dashboard")
            else if (data.role === "CUSTOMER") router.push("/costumer/dashboard")
        ...
        <div className="min-h-screen bg-gradient-to-br from-[#0A3F62] via-[#0F5B8C] to-[#2B7CB0] flex items-center justify-center p-4">
            <div className="w-full max-w-md">
                ...
                <div className="bg-white rounded-2xl shadow-2xl p-8">
                    ...
                    {/* ✅ Selalu render error container, toggle dengan CSS bukan conditional
                        Conditional render {error && <div>} bisa mismatch karena
                        server render kosong tapi client langsung render elemen */}
                    {error && (
                    ...
                    <p className="text-center text-sm text-gray-500 mt-6">
                        Belum punya akun?{" "}
                        <Link href="/sign-up" className="text-[#0F5B8C] font-medium hover:underline">
                            Daftar di sini
                        </Link>
                    </p>
                </div>
